...
\begin{proof}
Since $CA $ is a PDT edge there is a circle $c_1 $ with C and A on it's border which does not contain any node (so non two-hop-neighbour of C or A, either) of U.
Suppose we take another third Point d, which is not in the Set U, to identify the circle $c_1 $, which is the circle with $ACD $ on it's border.
...
If d moves on this line the angle $\alpha $ changes.  
...
d is continuous in the interval $[\beta,\pi[ $
