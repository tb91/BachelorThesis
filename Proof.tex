...
\item Since the angles $\alpha $ and $\beta $ between opposite points of a chord in a rectangle which corners lie on a circle are supplementary, this is a fact: $\angle{ACB}=\pi - \angle{ADB} $ (see figure \ref{fig:winkel_fuer_outward_3} for more details).
...
\caption{If four points $A $, $B $, $C $ and $M_1 $ are on one circle and $C $ and $M_1 $ are on different halfplanes of chord $AB $, then $\alpha + \beta =\pi $ is true.}
