...
The main argument of this proof is that $\bigcirc{CM_iM_p} $ is contained completely in the area $\triangle{ABC} \cup \bigcirc{CM_i} \cup R_{\bigcirc{M_iM_pM_j}} \cup R2_{\bigcirc{CM_iM_j}} $ with $R2_{\bigcirc{CM_iM_j}} $ being the area of $\bigcirc{CM_iM_j} $ subtended by chord $M_iM_j $ closer to $C $.
This time I show that $\bigcirc{CM_iM_p} $ cannot overlap $\bigcirc{CM_i} $ in the area subtended by chord $CM_i $ away from $M_j $.
Again, the proof is divided into three cases:
\begin{enumerate}
\item $\bigcirc{CM_iM_p} $ is equal to $\bigcirc{CM_i} $.
\item $\bigcirc{CM_iM_p} $ overlaps $\bigcirc{CM_i} $ in the halfplane subtended from line $CM_i $ away from $M_j $.
\item $\bigcirc{CM_iM_p} $ overlaps $\bigcirc{CM_i} $ in the halfplane subtended from line $CM_i $ closer to $M_j $.
\end{enumerate}

b worst case das ist der kreis aus a. derselbe ist wie der hier.    wenn er waechst  ist immer noch drin.   er kann nur auf einer seite herausgucken.
\end{proof}
\begin{figure}[h!]
\centering
